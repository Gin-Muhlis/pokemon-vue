<template>
    ...
    <div
        ...
        <button v-if="isDelete" @click="store.deleteCatchedPokemon(id)"
            class="btn-delete z-50 flex items-center justify-center absolute top-3 left-3 bg-red-500 w-5 h-5 rounded-full">

            <img src="/images/x.png" alt="X Image" class="x-delete w-4 object-cover">
        </button>

        <img v-if="isCatchedPokemon(number)" src="/images/pokeball1.png" alt="Poke ball left image"
            class="pokeball-catched w-5 object cover absolute left-3 top-3">
        ...
        <RouterLink :to="`/pokemon/${name}`">
            <div class="card-body py-10">
                <div class="flex flex-col items-start justify-start gap-1 relative z-20">
                    ...
                    <!-- nama pokemon -->
                    <p class="pokemon-name text-xl lg:text-2xl font-semibold text-slate-950 capitalize">{{ name
                        }}</p>
                    <p class="pokemon-nickname text-xl lg:text-lg italic font-semibold text-slate-950">{{ nickname }}
                    </p>
                </div>
